...
void do_combat(dungeon_t *d, character_t *atk, character_t *def)
{
  ...
  can_see_atk = can_see_def(d, &d->c, atk);
  can_see_def = can_see(d, &d->c, def);
...
void do_moves(dungeon_t *d)
{
  ...
  while (pc_is_alive(d) &&
         ...
         ((e->type != event_character_turn) || (e->c != &d->c))) {
    d->time = e->time;
...
uint32_t move_pc(dungeon_t *d, uint32_t dir)
{
  ...
  next[dim_y] = d->c.position[dim_y];
  next[dim_x] = d->c.position[dim_x];

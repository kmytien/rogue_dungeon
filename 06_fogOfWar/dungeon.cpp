...
int write_dungeon(dungeon_t *d, char *file)
{
  ...
  fwrite(&d->c.position[dim_x], 1, 1, f);
  fwrite(&d->c.position[dim_y], 1, 1, f);
...
int read_dungeon(dungeon_t *d, char *file)
{
  ...
  fread(&d->c.position[dim_x], 1, 1, f);
  fread(&d->c.position[dim_y], 1, 1, f);
...
void render_movement_cost_map(dungeon_t *d)
{
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      if (p[dim_x] ==  d->c.position[dim_x] &&
          p[dim_y] ==  d->c.position[dim_y]) {
        putchar('@');
      } else {
...
void new_dungeon(dungeon_t *d)
{
  ...
  place_pc(d);
  d->character_map[d->c.position[dim_y]][d->c.position[dim_x]] = &d->c;

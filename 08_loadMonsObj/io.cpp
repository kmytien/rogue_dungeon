...
static void io_redisplay_visible_monsters(dungeon *d)
{
         ...
         pos[dim_y]++)
    {
             ...
             pos[dim_x]++)
        {
            ...
            } else if (d->object_map[d->PC->position[dim_y] + pos[dim_y]]
                       [d->PC->position[dim_x] + pos[dim_x]] &&
                       (can_see(d, d->PC->position,
                                d->object_map[d->PC->position[dim_y] + pos[dim_y]]
                                [d->PC->position[dim_x] +
                                 pos[dim_x]]->position, 1, 0) ||
                        d->object_map[d->PC->position[dim_y] + pos[dim_y]]
                        [d->PC->position[dim_x] + pos[dim_x]]->shown))
            {
                attron(COLOR_PAIR(d->object_map[d->PC->position[dim_y] + pos[dim_y]]
                                  [d->PC->position[dim_x] +
                                   pos[dim_x]]->color));
                mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                        d->PC->position[dim_x] + pos[dim_x],
                        d->object_map[d->PC->position[dim_y] + pos[dim_y]]
                        [d->PC->position[dim_x] + pos[dim_x]]->obj_symbol());
                attroff(COLOR_PAIR(d->object_map[d->PC->position[dim_y] + pos[dim_y]]
                                   [d->PC->position[dim_x] +
                                    pos[dim_x]]->color));
            } else
            {
                                          ...
                                          [pos[dim_x]]), 1, 0))
            {
                ...
                mvaddch(pos[dim_y] + 1, pos[dim_x],
                        character_get_symbol(d->character_map[pos[dim_y]]
                                             [pos[dim_x]]));
                attroff(COLOR_PAIR(color));
            } else if (d->object_map[pos[dim_y]]
                       [pos[dim_x]] &&
                       (d->object_map[pos[dim_y]]
                        [pos[dim_x]]->shown ||
                        can_see(d, character_get_pos(d->PC), pos, 1, 0)))
            {
                attron(COLOR_PAIR(d->object_map[pos[dim_y]]
                                  [pos[dim_x]]->color));
                mvaddch(pos[dim_y] + 1, pos[dim_x],
                        d->object_map[pos[dim_y]]
                        [pos[dim_x]]->obj_symbol());
                attroff(COLOR_PAIR(d->object_map[pos[dim_y]]
                                   [pos[dim_x]]->color));
            } else
            {
                switch (pc_learned_terrain(d->PC,
    ...
    for (pos[dim_y] = 0; pos[dim_y] < DUNGEON_Y; pos[dim_y]++)
    {
        for (pos[dim_x] = 0; pos[dim_x] < DUNGEON_X; pos[dim_x]++)
        {
            ...
            } else if (d->character_map[pos[dim_y]][pos[dim_x]])
            {
                ...
                attroff(COLOR_PAIR(color));
            } else if (d->object_map[pos[dim_y]][pos[dim_x]])
            {
                attron(COLOR_PAIR(d->object_map[pos[dim_y]][pos[dim_x]]->color));
                mvaddch(pos[dim_y] + 1, pos[dim_x],
                        d->object_map[pos[dim_y]][pos[dim_x]]->obj_symbol());
                attroff(COLOR_PAIR(d->object_map[pos[dim_y]][pos[dim_x]]->color));
            }
            attroff(A_BOLD);
        }
    }

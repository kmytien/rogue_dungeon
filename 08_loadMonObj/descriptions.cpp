    ...
    for (f >> *lookahead, count = 0;
        ...
        if (*lookahead == "NAME") {
          ...
          if (read_attr || parse_object_attr(f, lookahead, &attr)) {
            ...
            read_rrty = true;
        } else { std::cerr << "Discovered at " << __FILE__ << ":" << __LINE__ << "\n"
                      << "Parse error in object description.\n"
                      << "Discarding object." << std::endl;
            return 1;
        }
...
static uint32_t parse_monster_descriptions(std::ifstream &f, dungeon *d, std::vector <monster_description> *v) {
    std::string s;
...
static uint32_t parse_object_descriptions(std::ifstream &f, dungeon *d, std::vector <object_description> *v) {
    std::string s;
    std::stringstream expected;
    std::string lookahead;
...
uint32_t parse_descriptions(dungeon *d) {
    ...
    if (parse_monster_descriptions(f, d, &d->mon_desc)) {
        retval = 1;
    }
    ...
    if (parse_object_descriptions(f, d, &d->obj_desc)) {
        retval = 1;
    }
...
uint32_t print_descriptions(dungeon *d) {
    std::vector <monster_description> &m = d->mon_desc;
    std::vector<monster_description>::iterator mi;
    std::vector <object_description> &o = d->obj_desc;
...
std::ostream &monster_description::print(std::ostream &o) { uint32_t i;
...
std::ostream &object_description::print(std::ostream &o) {
    ...
    return o << hit << std::endl << damage << std::endl << dodge << std::endl
             << defense << std::endl << weight << std::endl << speed << std::endl
...
npc *monster_description::gen_dyn_monster(dungeon *d) {

    npc *n;
    std::vector <monster_description> &foo = d->mon_desc;
    uint32_t i;

	//did this with help from piazza
    while (!foo[(i = (rand() % foo.size()))].generating() || !foo[i].rr_roll());

    monster_description &m = foo[i];
    n = new npc(d, m);

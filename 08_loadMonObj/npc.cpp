...
void gen_monsters(dungeon *d) { // WE NEED A SEP MD GEN FUNCTION BC EVERYTHING IN THAT CLASS IS PRIVATE
    uint32_t i;
    uint32_t c;

    if (d->max_monsters < (c = max_monster_cells(d))) d->num_monsters = d->max_monsters;
    else d->num_monsters = c; // ORIG MIN FUNCTION WASNT WORKING FOR ME SO SWITCHED TO IF ELSE why??? idk

    for (i = 0; i < d->num_monsters; i++) {
        monster_description::gen_dyn_monster(d); // similar to our object method CALL TO PRIVATE FUNCTION
    }
}
